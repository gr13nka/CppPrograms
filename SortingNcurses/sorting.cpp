...
void print_text(int y, int x,string texts) {
    const char* text = texts.c_str();// string to char* make
    mvprintw(y,x, text);
	refresh();
	echo();
}
void print_wait(int y, int x, string texts) 
{
    const char* text = texts.c_str();// string to char* make
    mvprintw(y,x, text);
	refresh();
...
void print_win(WINDOW *input_win)
{
	box(input_win,0,0);
	wrefresh(input_win);
}
...
{ 
    ...
    while (true) 
    {
        ...
        {
            print_wait(3,4, "Incorrect input");
            return 300;
        }
    }
...
{l.push_back("3");
    ...
    print_text(0, 0, "Use arrow keys to go up and down, Press enter to select a choice\n P.s. programm works ONLY with numbers");
    running = 1;
...
while(running)
{
	...
	print_list(list_win);
	print_text(0, 0, "Use arrows to go up and down, Press enter to select. \n P.s. programm works ONLY with numbers");
	c = wgetch(menu_win);
	switch(c)
	{	case KEY_UP:
			if(highlight == 1)
		...
		default:
			...
			print_text(0, 20, " instead of arrows?");
			break;
}
	...
	if (choice == 1)
	{
		while(1)
		{
            ...
            print_text(5,4,value);
            if (value  == "300"){
                goto f;
                break;
            }
            
            print_text(5,4, "Enter pos: ");
            
            position = get_longint();

            if ((position > l.len) || (position < 0))
            {
                print_wait(3,4, "list is too short ");
                goto f;
            }
    ...
    if (choice == 2)
	{
		while(1)
		{
            ...
            if (value.length()>9) 
            {
                print_wait(3,4, "Too big for int ");
                goto f;
            }
    ...
    if (choice == 4)
	{
		while(1)
		{
            ...
            print_text(3,4, "Enter start number: ");
         
            start = get_longint();

            print_text(5,4, "How many: ");

            amount= get_longint();

            print_text(7,4, "Where: ");

            position= get_longint();
           
            if ((amount < 0) || (amount + l.len > 20) || (start < 0) || (start + amount > l.len + 1) || (position < 0) || (position > l.len)|| (amount>l.len))
            {
                print_wait(9,4, "Incorrect options!!");
                goto f;
            }
    ...
    if (choice == 5)
	{
		while(1)
		{
            ...
            print_text(3,4, "Enter index: ");
            
            position = get_longint();

            if ((position > l.len) || (position < 0) || (position >= l.len))
            {
                print_wait(3,4, "Incorrect options!!!!!");
                goto f;         
            }
    ...
    if (choice == 6)
	{
		while(1)
		{
            ...
            print_text(3,4, "From which index delete?: ");
            ...
            if ((position + amount > l.len) || (amount < 0) || (position < 0)){
                print_wait(7,4, "Incorrect options ");
                goto f;
            }
